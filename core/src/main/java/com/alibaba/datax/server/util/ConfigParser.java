...
public final class ConfigParser {
    ...
    private static String getJobContent(String jobResource) {
        ...
        if (isJobResourceFromHttp) {
            ...
            try {
                ...
                throw DataXException.asDataXException(com.alibaba.datax.core.util.FrameworkErrorCode.CONFIG_ERROR, "获取作业配置信息失败:" + jobResource, e);
            }
        } else {
        ...
        if (jobContent == null) {
            throw DataXException.asDataXException(com.alibaba.datax.core.util.FrameworkErrorCode.CONFIG_ERROR, "获取作业配置信息失败:" + jobResource);
        }
        return jobContent;
    }
    ...
    public static Configuration parsePluginConfig(List<String> wantPluginNames) {
        ...
        if (wantPluginNames != null && wantPluginNames.size() > 0 && wantPluginNames.size() != complete) {
            throw DataXException.asDataXException(com.alibaba.datax.core.util.FrameworkErrorCode.PLUGIN_INIT_ERROR, "插件加载失败，未完成指定插件加载:" + wantPluginNames);
        }
