...
public class JobContainer extends AbstractContainer {
    ...
    private int split() {
        ...
        if(taskNumber==0)
        {
            LOG.warn("Task数目为0,可能是没有数据需求被传输.");
            return 0;
        }
        ...
        //LOG.info("合并后的配置如下: "+ JSON.toJSONString(contentConfig));
    ...
    /*
    把最终统计信息用JSON格式打印出来,方便提取数据传输结果,
    最终统计信息包括脏数据统计,便于数据稽核
     */
    private void logJson(long totalCosts,long totalReadRecords, long totalErrorRecords,long totalTransformerSuccessRecords,long totalTransformerFailedRecords,long totalTransformerFilterRecords, long byteSpeedPerSecond, long recordSpeedPerSecond, Communication communication)
    {
        JSONObject result=new JSONObject();
        result.put("totalCosts",totalCosts);
        result.put("totalReadRecordsAfterTransform",totalReadRecords);
        result.put("totalErrorRecords",totalErrorRecords);
        result.put("totalTransformerSuccessRecords",totalTransformerSuccessRecords);
        result.put("totalTransformerFailedRecords",totalTransformerFailedRecords);
        result.put("totalTransformerFilterRecords",totalTransformerFilterRecords);
        result.put("byteSpeedPerSecond",byteSpeedPerSecond);
        result.put("recordSpeedPerSecond",recordSpeedPerSecond);

        List<Configuration> transformerList = this.configuration.getListConfiguration(CoreConstant.DATAX_JOB_CONTENT_TRANSFORMER);
        if(transformerList!=null && !transformerList.isEmpty())
        {
            for(int idx=0; idx<transformerList.size(); idx++)
            {
                String filterKey="filterRecordsTransform_"+idx;
                String failedKey="failedRecordsTransform_"+idx;

                long filterRecords=communication.getLongCounter(filterKey);
                long failedRecords=communication.getLongCounter(failedKey);

                result.put(filterKey,filterRecords);
                result.put(failedKey,failedRecords);
            }
        }

        LOG.info("Final statistics in JSON: "+result.toString());
    }
    ...
    private void logStatistics() {
        ...
        long totalTransformerSuccessRecords=communication.getLongCounter(CommunicationTool.TRANSFORMER_SUCCEED_RECORDS);
        long totalTransformerFailedRecords=communication.getLongCounter(CommunicationTool.TRANSFORMER_FAILED_RECORDS);
        long totalTransformerFilterRecords=communication.getLongCounter(CommunicationTool.TRANSFORMER_FILTER_RECORDS);
        if (totalTransformerSuccessRecords > 0
                || totalTransformerFailedRecords > 0
                || totalTransformerFilterRecords > 0) {
            LOG.info(String.format(
                    "\n" + "%-26s: %19s\n" + "%-26s: %19s\n" + "%-26s: %19s\n",
                    "Transformer成功记录总数",
                    totalTransformerSuccessRecords,

                    "Transformer失败记录总数",
                    totalTransformerFailedRecords,

                    "Transformer过滤记录总数",
                    totalTransformerFilterRecords
            ));
        }

        logJson(totalCosts,totalReadRecords,totalErrorRecords,totalTransformerSuccessRecords,totalTransformerFailedRecords,totalTransformerFilterRecords,byteSpeedPerSecond,recordSpeedPerSecond,communication);
    ...
    private List<Configuration> doReaderSplit(int adviceNumber) {
        ...
        if (readerSlicesConfigs == null) {
            throw DataXException.asDataXException(
                    ...
                    "reader切分task操作错误,返回的readerSlicesConfigs为NULL.");
        }
