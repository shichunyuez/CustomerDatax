...
public class DFSUtil {
     ...
     */
    public HashSet<String> getAllFiles(List<String> srcPaths, String specifiedFileType,boolean isRecursive) {

        this.specifiedFileType = specifiedFileType;

        if (!srcPaths.isEmpty()) {
            for (String eachPath : srcPaths) {
                LOG.info(String.format("get HDFS all files in path = [%s], isRecursive = [%s]", eachPath, isRecursive));
                getHDFSAllFiles(eachPath,isRecursive);
            }
        }
    ...
    public HashSet<String> getHDFSAllFiles(String hdfsPath,boolean isRecursive) {
            ...
            if (hdfsPath.contains("*") || hdfsPath.contains("?")) {
                ...
                for (FileStatus f : stats) {
                    if (f.isFile()) {
                        ...
                        getHDFSAllFilesNORegex(f.getPath().toString(), hdfs, isRecursive);
                    }
                }
            } else {
                getHDFSAllFilesNORegex(hdfsPath, hdfs, isRecursive);
            }
    ...
    private HashSet<String> getHDFSAllFilesNORegex(String path, FileSystem hdfs,boolean isRecursive) throws IOException {
    ...
    暂时不可用
     */
    public void orcFileStartReadNew(String sourceOrcFilePath, Configuration readerSliceConfig,
                                    RecordSender recordSender, TaskPluginCollector taskPluginCollector)
    {
        LOG.info(String.format("New version, Start Read orcfile [%s].", sourceOrcFilePath));
        ...
        if (columnIndexMax >= 0) {
            ...
            try {
                ...
                org.apache.hadoop.hive.ql.io.orc.RecordReader rows = reader.rows();
                ...
                while (rows.nextBatch(batch)) {
                    ...
                    for(int r=0; r < batch.size; ++r) {
                        ...
                        {
                            recordFields.add(((BytesColumnVector)batch.cols[i]).vector[r]);
                        }
    ...
    public void orcFileStartRead(String sourceOrcFilePath, Configuration readerSliceConfig,
        ...
        if (columnIndexMax >= 0) {
            ...
            try {
                ...
                if(splits.length==0)
                {
                    LOG.warn(String.format("没有得到splits, 请确认目录下是否有数据, 或者目录下的文件是否有效. [%s]", sourceOrcFilePath));
                    return;
                }
