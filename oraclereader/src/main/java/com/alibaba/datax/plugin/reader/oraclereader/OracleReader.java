...
public class OracleReader extends Reader {
	...
	public static class Job extends Reader.Job {
		...
		public List<Configuration> split(int adviceNumber) {
			String directMode=originalConfig.getString(Key.DIRECT_MODE);

			if(directMode !=null && !directMode.equals("range") && !directMode.equals("union"))
			{
				throw DataXException
						.asDataXException(DBUtilErrorCode.REQUIRED_VALUE,
								String.format("您配置的 directMode 有误 [%s]，目前只支持range,union模式.",
										directMode));
			}
			List<Configuration> splitList=null;
			if(directMode!=null && (directMode.equals("range") || directMode.equals("union")))
			{
				splitList=OracleReaderDirectSplitUtil.doSplit(this.originalConfig,
						adviceNumber);
				LOG.info("Direct模式 "+directMode+" : Split完成,建议数量为: "+adviceNumber+", 实际数量为: "+splitList.size());
			}
			else
			{
				splitList=this.commonRdbmsReaderJob.split(this.originalConfig,
						adviceNumber);
